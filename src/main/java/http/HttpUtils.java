...
public class HttpUtils {
    ...
    // 超时时间，单位为秒
    public static final int connection_time = 20;
    public static final int reader_time = 30;

    private static OkHttpClient mOkHttpClient = null;

    private static OkHttpClient mOkHttpClientNoProxy = null;
    ...
    public static synchronized OkHttpClient clientNoProxy() {
        if (mOkHttpClientNoProxy == null) {
            ...
            try {
                mOkHttpClientNoProxy = new OkHttpClient.Builder()
                        .connectTimeout(connection_time, TimeUnit.SECONDS)
                        .readTimeout(reader_time, TimeUnit.SECONDS)
                        .sslSocketFactory(createSSLSocketFactory())
                        .hostnameVerifier(new HostnameVerifier() {
...
//    }
    ...
    public static ResponseWrap retryHttp(Request request) {
        return retryHttp(request, "UTF-8");
    }

    public static ResponseWrap retryHttp(Request request, String charset) {
...
//        return responseWrap;
//    }
    public static RequestBody createPostBody(final String write) {
        return new RequestBody() {
            ...
            public void writeTo(BufferedSink sink) {
                Buffer buffer;
                buffer = sink.buffer();
                buffer.writeString(write, Charset.forName("UTF-8"));
            }
        };
    }
}
