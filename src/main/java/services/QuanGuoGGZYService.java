                ...
                try {
                    ...
                    if (responseWrap.isSuccess()) {
                        ...
                        for (Element tenderEl : tenderElements) {
                            ...
                            String detailUrl = tenderEl.select("a").attr("href"); //用这个url来判断是否存在

                            String time = tenderEl.select(".span_o").get(0).ownText().trim();

                            Date timeDate = new SimpleDateFormat("yyyy-MM-dd").parse(time);

                            String yeWuLeiXing = tenderEl.select(".p_tw .span_on").get(2).ownText().trim(); //category
                            ...
                            int tit = dao.selecttitle(detailUrl);
                            if(tit == 1){
                                break loopPage;
                            }
