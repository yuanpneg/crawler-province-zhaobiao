 ...
 * 揭阳市
 */
public class JieyangService {

    private static final Logger LOGGER = Logger.getLogger(JieyangService.class); //使用log4j记录日志
    ...
    public List<Tender> getList(String url) {
        ...
        try {
            ...
            if (responseWrap.isSuccess()) {
                ...
                LOGGER.info("揭阳市 请求列表为空");
            }
        } catch (Exception e) {
            LOGGER.info("揭阳市 列表获取报错");
    ...
    public List<File> fileinfo(Document doc) {
        ...
        if (null != flo) {
            ...
            for (Element li : as) {
                File file = new File();
                String wUrl = "http://www.jysggzy.com" + li.select("a").attr("href");
                if (!wUrl.equals("")) {
